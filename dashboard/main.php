...
// Get generation counts from database using YEAR function
$generationSql = "
    SELECT 
        CASE 
    ...
    WHERE birthday IS NOT NULL
        ...
        AND YEAR(birthday) >= 1928 
        AND YEAR(birthday) <= 2012
    GROUP BY 
        CASE 
            WHEN YEAR(birthday) BETWEEN 1997 AND 2012 THEN 'Gen_Z'
            WHEN YEAR(birthday) BETWEEN 1981 AND 1996 THEN 'Millennials'
...
if ($generationResult) {
    while ($row = $generationResult->fetch_assoc()) {
        if (isset($generationCounts[$row['generation']])) {
            $generationCounts[$row['generation']] = (int)$row['count'];
        }
    }
}

// Add this debug code temporarily to check the values
error_log("Generation Counts: " . print_r($generationCounts, true));
...
if ($genderResult) {
    while ($row = $genderResult->fetch_assoc()) {
        if (isset($genderCounts[$row['gender']])) {
            $genderCounts[$row['gender']] = (int)$row['count'];
        }
    }
}

// Replace the existing barangay query with this optimized version
$barangayQuery = "SELECT 
    CASE 
        WHEN barangay LIKE '%san martin%' THEN 'San Martin De Porres'
        ELSE barangay 
    END as barangay,
    COUNT(*) as count 
FROM pop_data 
...
// Initialize barangay counts with exact names
$barangayCounts = [
    "BF Homes" => 0,
    "Don Bosco" => 0,
    "Marcelo Green" => 0,
...
if ($barangayResult) {
    while ($row = $barangayResult->fetch_assoc()) {
        $barangayName = trim($row['barangay']);
        if (isset($barangayCounts[$barangayName])) {
            $barangayCounts[$barangayName] = (int)$row['count'];
        }
    }
}

// Debug line - remove after testing
error_log("Barangay Counts: " . print_r($barangayCounts, true));

// Add this after the gender counts section
$ageSql = "SELECT age, COUNT(*) as count 
           FROM pop_data 
...
$ageDistribution = array_fill(0, 101, 0); // Initialize array for ages 0-100

if ($ageResult) {
    while ($row = $ageResult->fetch_assoc()) {
        $age = (int)$row['age'];
        if ($age >= 0 && $age <= 100) {
            $ageDistribution[$age] = (int)$row['count'];
        }
    }
}
?>
...
<?php if (isset($_SESSION['toast'])): ?>
    <div class="toast-container position-fixed top-0 end-0 p-3">
        <div class="toast show" role="alert" aria-live="assertive" aria-atomic="true">
            <div class="toast-header <?= $_SESSION['toast']['type'] === 'error' ? 'bg-danger text-white' : 'bg-success text-white' ?>">
                <strong class="me-auto">
                    <?= $_SESSION['toast']['type'] === 'error' ? 'Error' : 'Success' ?>
                </strong>
...
<?php endif; ?>
    ...
    <div class="gender-chart-card">
        ...
        <!-- Replace the existing gender legend -->
        <div class="gender-legend">
            ...
            <div class="legend-item">
                <div class="legend-color" style="background: rgba(147, 51, 234, 0.8)"></div>
                <span>LGBTQ+</span>
            </div>
        </div>
    </div>
        ...
        <!-- Add this after the gender chart div -->
<div class="row mb-4">
    <div class="col-12">
        <div class="chart-card">
            <div class="d-flex justify-content-between align-items-center mb-4">
                ...
                <div class="btn-group" role="group" aria-label="Chart type toggle">
                    <button type="button" class="btn btn-primary active" data-age-chart="bar">Bar</button>
                    <button type="button" class="btn btn-primary" data-age-chart="line">Line</button>
                    <button type="button" class="btn btn-primary" data-age-chart="area">Area</button>
                </div>
            </div>
            <div class="chart-container">
...
<div id="chartError" style="display:none;" class="alert alert-danger">
    ...
    console.log('Barangay Data:', chartData.barangayLabels, chartData.barangayData);
</script>
    ...
    <!-- Add this script for age histogram -->
<script>
    ...
    function initAgeChart(type = 'bar') {
        ...
        const gradientColors = [
            ...
            'rgba(59, 131, 246, 1)',  // Blue
            'rgba(0, 196, 130, 1)',  // Green
            'rgba(245, 159, 11, 1)',  // Orange
            'rgba(42, 0, 141, 1)'   // Purple
        ];

        // Create color arrays for all data points
        const backgroundColors = ageData.map((_, index) => 
            gradientColors[index % gradientColors.length]
        );

        const borderColors = ageData.map((_, index) => 
            gradientColors[index % gradientColors.length].replace('0.7', '1')
        );
            ...
            data: {
                ...
                datasets: [{
                    ...
                    backgroundColor: type === 'line' ? 
                        'rgba(99, 102, 241, 0.5)' : backgroundColors,
                    borderColor: type === 'line' ? 
                        'rgba(99, 102, 241, 1)' : borderColors,
                    borderWidth: 1,
                    ...
                    tooltip: {
                        callbacks: {
                            title: function(tooltipItems) {
                                return `Age: ${tooltipItems[0].label} years`;
                            },
                            label: function(context) {
                                return `Count: ${context.raw} residents`;
                            }
                        }
                    }
                },
        ...
        ageHistogram = new Chart(ctx, config);
    }
    ...
    document.querySelectorAll('[data-age-chart]').forEach(button => {
        button.addEventListener('click', function() {
            // Remove active class from all buttons
            document.querySelectorAll('[data-age-chart]').forEach(btn => {
            ...
            // Update chart type
            const chartType = this.getAttribute('data-age-chart');
            initAgeChart(chartType === 'area' ? 'line' : chartType);
        });
    });
</script>
